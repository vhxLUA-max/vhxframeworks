...
local function getNearestEnemy()
    ...
    for i=1,_enemyCount do
        ...
        if e.humanoid and e.humanoid.Health>0 and e.root and _damageable[e.model] then
            local dx,dy,dz=myPos.X-e.root.Position.X,myPos.Y-e.root.Position.Y,myPos.Z-e.root.Position.Z
...
Cn(Run.Heartbeat:Connect(function(dt)
    ...
    if _wsDirty and _wsCacheTimer >= 0.5 then _wsCacheTimer=0; _wsDirty=false; _rebuildWSCache() end

    -- Enemy refresh (0.5s)
    _refreshTimer = _refreshTimer + dt
    if _refreshTimer >= 0.5 then _refreshTimer=0; _refreshEnemies() end
